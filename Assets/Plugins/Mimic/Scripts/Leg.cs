...
namespace MimicSpace
{
    public class Leg : MonoBehaviour
    {
        ...
        public void Initialize(Vector3 footPosition, int legResolution, float maxLegDistance, float growCoef, Mimic myMimic, float lifeTime, LayerMask layerMask, bool isForce)
        {
            ...
            {
                RaycastHit hit;
                Physics.Raycast(footPosition + Vector3.up * 5f + new Vector3(footOffset.x, 0, footOffset.y),
                    -Vector3.up, out hit, float.PositiveInfinity, _layerMask);
                handles[7] = hit.point;
            }
            else handles[7] = footPosition;
            ...
            if (lifeTime != -1f)
            {
                ...
                StartCoroutine(nameof(WaitAndDie), lifeTime);
            }
        ...
        void Sethandles()
        {
            ...
            if (_isForce) handles[7] = footPosition;
        }
