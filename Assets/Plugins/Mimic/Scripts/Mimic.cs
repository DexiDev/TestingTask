    ...
    public class Mimic : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (legCount <= maxLegs - partsPerLeg)
            {
                ...
                RaycastHit hit;
                Physics.Raycast(newLegPosition + Vector3.up * 10f, -Vector3.up, out hit, float.PositiveInfinity, _layerMask);
        ...
        private Leg RequestLeg(Vector3 footPosition, int legResolution, float maxLegDistance, float growCoef, Mimic myMimic, float lifeTime, bool isForce = false)
        {
            Leg newLeg;
            ...
            newLeg.Initialize(footPosition, legResolution, maxLegDistance, growCoef, myMimic, lifeTime, _layerMask,isForce);
            newLeg.transform.SetParent(myMimic.transform);

            return newLeg;
        }

        public Leg RequestLeg(Vector3 position)
        {
            return RequestLeg(position, legResolution, 1000f, Random.Range(minGrowCoef, maxGrowCoef), this,
                -1f, true);
        }
